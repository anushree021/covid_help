...
if(isset($_POST['update'])) #isset-to check whether update has been clicked or not, $_POST to access the name update
{
	...
	$query="update medicine set med_name='".$_POST['name']."', descrip='".$_POST['descrip']."', symptom_id='".$_POST['category']."', price='".$_POST['price']."', image='".$file."' where med_id='".$_REQUEST['id']."'";
	mysqli_query($con,$query);
}

$query="select * from medicine where med_id='".$_REQUEST['id']."'";
$result=mysqli_query($con,$query);
$fetch=mysqli_fetch_object($result);

$query_c="select * from symptom where status='1'";
...
<body>
	<div class="menubar">
	    ...
	    <div class="item"><a href="medicine.php">Go to Medicine table</a></div>
    </div>
	...
	<form method="post" action="" enctype="multipart/form-data"> <!-- method="get(not secured)/post(secured)"-->
		<table>
			<tr>
				<td>Medicine Name : </td>
				<td><input type="text" name="name" value="<?php echo $fetch->med_name;?>"></td>
			</tr>
			<tr>
				<td>Description : </td>
				<td><input type="text" name="descrip" value="<?php echo $fetch->descrip;?>"></td>
			</tr>
			<tr>
				...
				<td>
					<select name="category">
						...
						<option value="<?php echo $fetch_c->id?>" <?php if($fetch->symptom_id==$fetch_c->id) {?> selected="selected" <?php }?> > <?php echo $fetch_c->name?> </option>
					    <?php }?>
					</select>
				</td>
			</tr>
