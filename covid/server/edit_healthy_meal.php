...
if(isset($_POST['update'])) #isset-to check whether update has been clicked or not, $_POST to access the name update
{
	...
	$query="update healthy_meal set hm_name='".$_POST['hm_name']."', c_id='".$_POST['category']."', descrip='".$_POST['descrip']."', price='".$_POST['price']."', image='".$file."' where hm_id='".$_REQUEST['id']."'";
	mysqli_query($con,$query);
}

$query="select * from healthy_meal where hm_id='".$_REQUEST['id']."'";
$result=mysqli_query($con,$query);
$fetch=mysqli_fetch_object($result);

$query_c="select * from meal_category where status='1'";
...
<body>
	...
	<form method="post" action="" enctype="multipart/form-data"> <!-- method="get(not secured)/post(secured)"-->
		<table>
			<tr>
				<td>Meal Name : </td>
				<td><input type="text" name="hm_name" value="<?php echo $fetch->hm_name;?>"></td>
			</tr>
			<tr>
				<td>Meal Category : </td>
				<td>
